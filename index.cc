...
std::string stringToHex(const unsigned char* input, int len) {
  if (!input) return "";
  ...
  for (int i = 0; i < len; i++) {
    ss << std::setw(2) << static_cast<int>(uInput[i]) << " ";
  }

  return ss.str();
}

std::string intsToString(std::vector<unsigned int> abc, int len) {
  if (abc.size() == 0 || len == 0) return "";

  std::stringstream ss;
  ss << std::hex;// << std::setfill('0');
  
  for (int i = 0; i < len; i++) {
    ss << static_cast<int>(abc[i]) << " ";
  }
...
void Index::ProcessRaw(unsigned char* raw_pkt, int64_t packet_offset, uint32_t epb_len) {
  ...
  if (epb_len < 0) {
    std::string hex_epb = stringToHex(raw_pkt, epb_len);
    LOG(INFO) << "Raw packet:\n" << hex_epb << "\n";
  }
